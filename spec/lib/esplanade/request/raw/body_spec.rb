...
RSpec.describe Esplanade::Request::Raw::Body do
  ...
  let(:raw_request) { double(method: 'method', path: 'path', raw_path: 'path', content_type: 'application/json') }
  let(:env) { double }
  ...
  describe '#to_hash' do
    ...
    context 'body nil' do
      let(:message) { '{:method=>"method", :path=>"path", :raw_path=>"path", :content_type=>"application/json", :body=>nil}' }
    ...
    context 'body is empty' do
      let(:message) { '{:method=>"method", :path=>"path", :raw_path=>"path", :content_type=>"application/json", :body=>" "}' }
      before { allow(subject).to receive(:to_string).and_return(' ') }
      it { expect { subject.to_hash }.to raise_error(Esplanade::Request::BodyIsNotJson, message) }
    end

    context 'can not parse body of request' do
      let(:message) { '{:method=>"method", :path=>"path", :raw_path=>"path", :content_type=>"application/json", :body=>"{\"state\": 1"}' }
      before { allow(subject).to receive(:to_string).and_return('{"state": 1') }
      it { expect { subject.to_hash }.to raise_error(Esplanade::Request::BodyIsNotJson, message) }
    end
  end
