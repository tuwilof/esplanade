...
RSpec.describe Esplanade::Response::Raw::Body do
  ...
  describe '#to_hash' do
    ...
    context 'can not parse body of response' do
      ...
      let(:message) { '{:request=>{:method=>"method", :path=>"path", :raw_path=>"path"}, :status=>"status", :body=>"body"}' }
      before { allow(subject).to receive(:to_string).and_return('{"state": 1') }
      it { expect { subject.to_hash }.to raise_error(Esplanade::Response::BodyIsNotJson, message) }
    end
  end
end
